...
const currentSetupExamples = (
  ...
  if (unitCountForPoint && pointCount) {
    if (units === DurationUnits.MINUTE) {
      ...
      if (pointCount !== 1) {
        examples.push(
          `Get ${1} point for ${(unitCountForPoint / pointCount).toFixed(
            2,
          )} minutes of "${taskName}"`,
        );
      }
      examples.push(
        `Get ${pointCount.toFixed(2)} point for ${unitCountForPoint.toFixed(
          2,
        )} minutes of "${taskName}"`,
      );
      examples.push(
        `Get ${(pointCount * 2).toFixed(2)} points for ${(unitCountForPoint * 2).toFixed(
          2,
        )} minutes of "${taskName}"`,
      );
      examples.push(
        `For one hour of "${taskName}", get ${((60 / unitCountForPoint) * pointCount).toFixed(
          2,
        )} points`,
      );
      examples.push(
        `For one minute of "${taskName}", get ${((1 / unitCountForPoint) * pointCount).toFixed(
          2,
        )} points`,
      );
    }
