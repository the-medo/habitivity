...
export const apiTaskList = apiSlice
  .enhanceEndpoints({ addTagTypes: ['TaskGroup'] })
  .injectEndpoints({
    endpoints: builder => ({
      ...
      >({
        queryFn: async ({ newTaskGroups, taskListId }, api) => {
          ...
          try {
            ...
            newTaskGroups.forEach(tg => {
              const taskGroupRef = doc(
                db,
                '/Users/' + userId + '/TaskGroups/' + tg.id,
              ).withConverter(taskGroupConverter);
        ...
        invalidatesTags: (result, error, arg) =>
          result
            ? [
                ...result.newTaskGroups.map(
                  ({ id }) => ({ type: 'TaskGroup', id: `single-${id}` } as const),
                ),
      ...
      updateTaskGroups: builder.mutation<
        { arrayOfTaskGroups: string[]; taskListId: string },
